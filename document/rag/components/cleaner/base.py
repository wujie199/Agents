...
class UnicodeNormalizerCleaner(BaseCleaner):
    """NFKC Unicode normalization."""

    def __init__(self):
        super().__init__("unicode_normalizer")
...
def dedupe_header_footer_from_pages(
    pages: List[Dict[str, Any]],
    threshold: float = 0.3,
) -> List[Dict[str, Any]]:
    """Remove lines repeated on more than ``threshold`` fraction of pages."""
    ...
    for page in pages:
        ...
        for line in content.split("\n"):
            ...
            if len(normalized) < 3 or normalized in seen_on_page:
                continue
    ...
    for page in pages:
        content = page.get("content", "") or ""
        kept = [ln for ln in content.split("\n") if ln.strip() not in repeat_lines]
        new_page = dict(page)
        new_page["content"] = "\n".join(kept)
