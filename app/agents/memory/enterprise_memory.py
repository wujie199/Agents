...
async def confirm_pending_l1(
    ctx: RunContext,
    *,
    chat_cfg: Any = None,
) -> int:
    """确认 pending L1 记忆，写入前检查冲突。

    Args:
        ctx: RunContext
        chat_cfg: ChatAgentConfig（用于冲突策略，默认自动加载）
    """
    memory = ctx.require_memory()
    ...
    # ── 冲突检测：确认前检查 pending 与现有 L1 是否冲突 ──
    try:
        ...
        if pending:
            ...
            conflicts = [r for r in records if r.needs_hitl and r.resolved_value != (r.new_value or "")]
            if conflicts:
                import logging
                logging.getLogger(__name__).warning(
                    "confirm_pending_l1: %d conflicts detected, keeping old values for: %s",
                    len(conflicts),
