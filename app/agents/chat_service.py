...
from app.agents.react_loop import _resolve_turn_buffer
...
async def _stream_direct_token_turn(
    ...
    *,
    ...
    messages, ev_count, rag_empty, _mem_hash = await build_turn_messages(
        ctx,
        message,
        cfg,
        enable_rag=use_rag,
    ...
    parts: list[str] = []
    async for delta in stream_llm_text(llm, messages):
        parts.append(delta)
